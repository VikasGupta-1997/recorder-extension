...
function PreviewPage() {
    ...
    const [blobUrl, setUrlBlob] = useState('')
    ...
    const [isBlobReady, setIsBlobReady] = useState(null)
    ...
    useEffect(() => {
        chrome.storage.local.get(["blobUrl", "isAudioOnly"], async (result) => {
            setIsAudio(result?.isAudioOnly);
            if (result?.isAudioOnly) {
                // getBlobFromBlobUrl(result?.blobUrl);
                setIsAudio('audio');
            } else {
                setIsAudio('video');
            }
            setUrlBlob(result?.blobUrl);
    ...
    async function getBlobFromBlobUrl(blobUrl) {
        ...
        try {
            console.log("FETCHH ISNIDE")
            const response = await fetch(blobUrl);
            console.log("Blobbb", response)
            const blob = await response.blob(); // Converts the blob URL back into a Blob object.
            setBlobUrlRef(blob);
            console.log("blob13121", blob)
            // initDB().then(async () => { 
            //   console.log("blob13121", blob)
            //   await storeAudio('currentAudio', blob, { isAudioOnly: true });
            // })
        } catch (error) {
            // const audioData = await getAudio('currentAudio');
            // if (audioData) {
            //   console.log("audioData11212", audioData)
            //   setBlobUrlRef(audioData.audioBlob);
            // }
    ...
    useEffect(() => {
        ...
        if (isAudio === 'video') {
            ...
            videoRef.current.addEventListener('loadeddata', (e) => {
                ...
                console.log("INSIDe  REf Load", videoRef.current.readyState)
